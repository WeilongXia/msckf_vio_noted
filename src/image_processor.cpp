...
ImageProcessor::ImageProcessor(ros::NodeHandle &n)
    : nh(n), is_first_img(true),
      // img_transport(n),
      stereo_sub(10), prev_features_ptr(new GridFeatures()), curr_features_ptr(new GridFeatures())
{
    return;
}
...
bool ImageProcessor::createRosIO()
{
    ...
    cam0_img_sub.subscribe(nh, "/camera/infra1/image_rect_raw", 10);
    cam1_img_sub.subscribe(nh, "/camera/infra2/image_rect_raw", 10);
